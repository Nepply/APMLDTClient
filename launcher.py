...
# Provide a minimal standalone handler so this apworld can run without the
# external `mldt` package. If the real `mldt.client.MLDTClient` is available
# we'll use it; otherwise this fallback offers `validate_rom` and
# `game_watcher` hooks that keep the launcher functional.
class StandaloneMLDTClient:
    ...
    # minimal state copied from upstream MLDTClient
    ram_offset = 0
    prev_data = 0
    prev_shop = 0
    shop_on = False
    ...
    # Tracks whether we were connected/authenticated to the AP server on the
    # previous pass, so we can detect reconnects (as opposed to emulator
    # reconnects, which reset_state() already covers) and re-scan for any
    # locations the server thinks are still missing.
    ...
    # location mapping (copied from upstream client). For brevity and to keep
    # parity with the real client, this is a direct copy of the mapping used
    # by `mldt.client`.
    ...
    async def game_watcher(self, ctx) -> None:
        ...
        while not ctx.exit_event.is_set():
            try:
                ...
                # Detect (re)connecting to the AP server -- as opposed to an emulator
                # reconnect, which reset_state() already handles -- and clear the
                # "already sent" trackers so the scans below re-check current game
                # state against the freshly (re)synced ctx.missing_locations, the same
                # way a full client restart would.
                ...
                # Poll the file-loaded flag first, since item syncing below needs to
                ...
                if file_has_loaded > 0:
                    if self.prev_data == 0:
                        # Legacy MLDT initializes `current_items_received` from the game memory
                        # instead of assuming the client starts at zero. This prevents replaying
                        # every server item after a reconnect or client launch.
